...
When a Distribution Set is assigned to a target in IoT Rollouts, IoT Manager will translate this into an install command on the device shadow in case that the Device shadow has a [SoftwareUpdatable][Hawkbit Vorto Functionblock] Vorto Function Block. So step 1 is registering the SoftwareUpdateable Functionblock. Let's extend the connect function to update the Device shadow.
...
The key elements that we will need to process are the path property, the correlationId of the software installation request *value.arg_0.correlationId* and the url to the artifact *value.arg_0.softwareModules.[].artifacts.[].links.download.https*. Let's add the handling code with some global variables for processing the message.
